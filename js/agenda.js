$(function(){
	...
	$('form').ajaxForm({
		...
		success:function(data){
			$('.box-alert').remove();
			$('form .card-title').after('<div class="box-alert sucesso">Evento adicionado.</div>');
			$('.box-tarefas .card-title').after('<div class="box-tarefas-single"><h2><i class="far fa-calendar"></i> '+data.tarefa+'</h2></div>');
			$('form')[0].reset();
		}
	})
	...
	function aplicarEventos(data){
		...
		$.ajax({
			url: include_path+'ajax/agenda.php',
			method:'post',
