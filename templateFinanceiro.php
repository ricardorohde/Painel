...
?>
<h2><i class="fas fa-file-invoice-dollar"></i> Pagamentos <?php echo $nome; ?></h2>
	...
	<table>
		...
		<?php
			if($nome == 'Pendentes')
				...
				$nome = 1;
			$sql = MySql::conectar()->prepare("SELECT * FROM `admin_financeiro` WHERE status = $nome ORDER BY vencimento ASC");
			$sql->execute();
