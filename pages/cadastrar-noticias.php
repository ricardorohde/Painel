    ...
    <form method="post" enctype="multipart/form-data">
        <?php
            if(isset($_POST['acao'])){
                ...
                if($titulo == '' || $conteudo == ''){
                    Painel::alert('erro','Os campos não podem ficar vazios.');
                }else if($capa['tmp_name'] == ''){
                    Painel::alert('erro','Selecione uma imagem capa.');
                }else{
                    if(Painel::imagemValida($capa)){
                        $verificar = MySql::conectar()->prepare("SELECT * FROM `site_noticias` WHERE titulo=? AND categoria_id = ?");
                        $verificar->execute(array($titulo,$categoria_id));
                        if($verifica->rowCount() == 0){
                            $imagem = Painel::uploadFile($capa);
                            $slug = Painel::generateSlug($titulo);
                            $arr = ['categoria_id'=>$categoria_id,'data'=>date('Y-m_d'),'titulo'=>$titulo,'conteudo'=>$conteudo,'capa'=>$imagem,'slug'=>$slug,'order_id'=>'0','nome_tabela'=>'site_noticias'];
                            if(Painel::insert($arr)){
                                Painel::redirect(INCLUDE_PATH_PAINEL.'cadastrar-noticias?sucesso');
                            }
                                
                            }else{
                                Painel::alert('erro','Título já existe.');
                        }
                    }else{
		...
		<div class="form-group">
			...
			<input type="hidden" name="nome_tabela" value="site_noticias" />
