<?php
	if(isset($_GET['id'])){
		$id = (int)$_GET['id'];
		$cliente = Painel::select('admin_clientes','id = ?',array($id));
	}else{
...
<div class="box-content">
	...
	<?php
		if(isset($_POST['acao'])){
			...
			for($i = 0; $i < $numero_parcelas; $i++){
				...
				$sql = MySql::conectar()->prepare("INSERT INTO `admin_financeiro` VALUES (null,?,?,?,?,?)");
	...
	<table>
		...
		<?php
			$sql = MySql::conectar()->prepare("SELECT * FROM `admin_financeiro` WHERE status = 0 AND cliente_id = $id ORDER BY vencimento ASC");
			$sql->execute();
		...
		<tr <?php echo $style; ?>>
			...
			<td><a class="btn" href="<?php echo INCLUDE_PATH_PAINEL ?>editar-clientes?id=<?php echo $id; ?>&pago=<?php echo $value['id'] ?>"><i class="fa fa-check"></i> Pago</a></td>
		</tr>
		<?php } ?>
	</table>
	</div>
	...
	<table>
		...
		<?php
			$sql = MySql::conectar()->prepare("SELECT * FROM `admin_financeiro` WHERE status = 1 AND cliente_id=$id ORDER BY vencimento ASC LIMIT 10");
			$sql->execute();
			$pendentes = $sql->fetchAll();
